...
class Telegram::SendAttachmentsService
  ...
  def send_file(chat_id, file_path, reply_to_message_id)
    File.open(file_path, 'rb') do |file|
      ...
      Rails.logger.info '=== TELEGRAM SEND DOCUMENT REQUEST ==='
      Rails.logger.info "URL: #{channel.telegram_api_url}/sendDocument"
      ...
      Rails.logger.info "Reply To Message ID: #{reply_to_message_id}"
      Rails.logger.info '========================================'

      response = HTTParty.post("#{channel.telegram_api_url}/sendDocument",
                               body: request_body,
                               multipart: true)

      Rails.logger.info '=== TELEGRAM SEND DOCUMENT RESPONSE ==='
      Rails.logger.info "Status: #{response.code}"
      Rails.logger.info "Response: #{response.body}"
      Rails.logger.info '========================================'
